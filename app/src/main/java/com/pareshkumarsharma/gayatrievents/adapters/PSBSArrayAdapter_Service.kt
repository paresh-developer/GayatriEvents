...
internal class PSBSArrayAdapter_Service(
    val c: Context,
    val r: Int,
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        checkBox?.isChecked = selections.contains(data[position][1])

        checkBox?.setOnCheckedChangeListener { compoundButton, b ->
            if(b){
                if(!selections.contains(data[position][1])){
                    selections.add(data[position][1])
                }
            }
            else{
                if(selections.contains(data[position][1])){
                    selections.remove(data[position][1])
                }
            }
        }
//        if(position==0){
